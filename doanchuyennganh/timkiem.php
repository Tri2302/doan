...
$sizes = $conn->query("SELECT * FROM kichco")->fetch_all(MYSQLI_ASSOC);
...
<body>
    ...
    <div class="container mt-4">
        <h2><?= htmlspecialchars($_SESSION['last_search'] ?? ''); ?></h2>
                ...
                <?php while ($product = $result->fetch_assoc()): ?>
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            ...
                            <img src='<?= file_exists($imagePath) ? htmlspecialchars($imagePath) : "img/default.png" ?>' class='card-img-top' alt='<?= htmlspecialchars($product["tensp"]) ?>'>
                            <div class="card-body">
    ...
    document.querySelectorAll('card').forEach(wrapper => {
        ...
        function update() {
            const sizemPrice = parseInt(sizem.options[sizem.selectedIndex].getAttribute('data-price')) || 0;
            ...
            const total = (parseInt(totalElement.dataset.basePrice) + sizemPrice + totalToppings) * quantity;
